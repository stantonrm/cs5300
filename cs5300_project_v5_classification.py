# -*- coding: utf-8 -*-
"""cs5300_project_v5.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1GzLbeeIFMKlpi7zwxxGMDG8lVy_J6A1e

# CS5300 Project Version 5: Classification
Genre information included.
Using only movies from USA from the last 20 years to keep the comparisons closer.
"""
...
import numpy as np
dataset = np.loadtxt('movie_metadata_usa_allgenres_modern_classification.csv', skiprows = 1,  delimiter=',')
...
plt.xlabel('Last feature - Crime Genre')
plt.ylabel('made money?')
plt.show()
...
trainset_output = trainset[:, dataset_features:dataset_features + 1]
validset_input = validset[:, 0:dataset_features]
validset_output = validset[:, dataset_features:dataset_features + 1]
...
model = Sequential()
model.add(Dense(1, input_dim = dataset_features, activation='sigmoid'))
print(model.summary())
model.compile(loss='binary_crossentropy', optimizer='sgd', metrics=['accuracy'])
# Verbose = 0 shows no updates, can be changed to 1 or 2
history = model.fit(trainset_input, trainset_output, epochs= 64, verbose = 1, batch_size=16, validation_data = (validset_input, validset_output))
...
# Verbose = 0 shows no updates, can be changed to 1 or 2
history = model.fit(trainset_input, trainset_output, epochs= 128, verbose = 0, batch_size=16, validation_data = (validset_input, validset_output))
